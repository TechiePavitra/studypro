...
els.topCsvInput.addEventListener('change', (ev) => {
  ...
  Papa.parse(f, {
    ...
    complete: (res) => {
      ...
      refreshQuestionList();
      alert('CSV imported: ' + res.data.length);
    }
  });
});
...
els.pdfFileInput.addEventListener('change', async (e) => {
  ...
  renderPDFIntoContainer(buf, els.pdfViewerContainer);
});

// drag & drop
els.pdfViewerContainer.addEventListener('dragover', (ev) => { ev.preventDefault(); ev.dataTransfer.dropEffect = 'copy'; });
els.pdfViewerContainer.addEventListener('drop', async (ev) => {
  ev.preventDefault(); const f = ev.dataTransfer.files[0]; if (!f) return;
  const buf = await f.arrayBuffer(); renderPDFIntoContainer(buf, els.pdfViewerContainer);
});
